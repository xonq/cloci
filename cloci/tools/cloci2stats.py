...
from mycotools.lib.kontools import collect_files, format_path, findExecs, mkOutput
...
def count_transcripts(gff_path):
    ...
    for line in gff:
        ...
        #            alias = alia_comp.search(line['attributes'])[1]
            ...
            if not error:
                logger.warning(f"{gff_path} missing alia")
            error = True
            continue
        if alias:
...
def calc_stats(ome, f, gff_path, alia=None):
            ...
            if not line.startswith("#"):
                try:
                    ...
                    if len(data) > 4:  # there are annotations
                        ...
                        top_anns = [
                            x.split("|")[0] for x in raw_annotations.split(";") if x
                        ]
                ...
                try:
                    ...
                    logger.warning(f"no HGs for: {line}")
                    hgs = []
                genes = genes.split(",")
    ...
    return ome, hlgs, mean_gic, median_gic, perc_clus, tot_gic, alia, alpha, genes2hg
...
def alien_mngr(genes_in_clus, algn_dir, ome2tax, cpus=1):
    ...
    with mp.Pool(processes=cpus) as pool:
        alien_res = pool.starmap(
            hg2alien,
            tqdm(
                (
                    (genes, f"{algn_dir}{hg}.out", ome2tax)
...
def summarize_stats(ome_stats, rank, hlg2data, db, tmp_path, ass_stats):
    ...
    with open(tmp_path, "w") as out:
        out.write(
            "tip\ttmd\tgcl\tmmi\tmmp\tcsb\tpds\tpc"
        ...
        if rank == "ome":
            for (
                ...
                try:
                    out.write(
                        f'{ome}\t{sum(tax_dict[ome]["tmd"])/len(tax_dict[ome]["tmd"])}\t'
            ...
            for (
                ...
                try:
                    out.write(
                        f"{ome}\t{sum(tmd)/len(tmd)}\t{sum(gcl)/len(gcl)}\t"
...
def output_stats(out_file, tax_dict, run_alien=False):
    ...
    with open(out_file + ".mean.tsv", "w") as out0, open(
        out_file + ".median.tsv", "w"
    ) as out1:
        if run_alien:
            out0.write(
                "#taxon\ttmd\tgcl\tmmi\tmmp\tcsb\tpds"
        ...
        for tax, data in tax_dict.items():
            if len(data["tmd"]) == 0:
                logger.warning(f"\t{tax} has no results")
                continue
...
def main(
    ...
    if gamma:
        ...
        if to_run:
            logger.info(f"Annotating {len(to_run)} full genomes")
            failed_omes = run_hmmsearch(pfam, to_run, ann_dir, cpus, db=db)
            passing_omes = sorted(set(db.keys()).difference(set(failed_omes)))

            ann_res = compile_hmm_res(ann_dir, passing_omes, cpus=cpus, max_hits=1)

    logger.info("Preparing assembly statistics")
    assembly_stats(mtdb_path, log_path=out_dir + "assembly_stats.tsv", cpus=cpus, db=db)
    ...
    logger.info("Collecting data for omes")
    ...
    if not gcf_only:
        ...
        if not os.path.isfile(primary_hlg):
            logger.error("`hlgs.tsv.gz` not detected")
            sys.exit(1)

        logger.info("Assimilating proxy values for HLGs")
        hlg2data = assimilate_proxies(primary_hlg)

        logger.info("Collecting stats by ome")
        with mp.Pool(processes=cpus) as pool:
            ome_stats = pool.starmap(
                calc_stats,
                ((ome, ome_dir + ome + "/hlg.tsv", db[ome]["gff3"]) for ome in omes),
            )

        logger.info("Summarizing data by " + rank)
        tmp_file = out_dir + f".cloci2summary.hlg.{rank}.tsv"
        hlg_stats, ome2alia, genes_in_clus = summarize_stats(
            ome_stats, rank, hlg2data, db, tmp_file, ome2ass_stats
        )
        #    if os.path.isfile(tmp_file):
        #       os.remove(tmp_file)
        if alien:
            logger.info("Calculating HLG alienness")
    ...
    if not os.path.isfile(primary_gcf):
        logger.warning("`gcfs.tsv.gz` not detected; ignoring GCFs")
    #       gcf2data = []
    else:
        logger.info("Assimilating proxy values for GCFs")
        ...
        with mp.Pool(processes=cpus) as pool:
            if ome2alia:
                ...
                ome_stats = pool.starmap(
                    ...
                    tqdm(
                        (
                            (ome, f"{ome_dir}{ome}/gcf.tsv", db[ome]["gff3"])
                            for ome in omes
                        ),
                        total=len(omes),
                    ),
        ...
        gcf_stats, null, genes_in_clus = summarize_stats(
            ome_stats, rank, hlg2data, db, tmp_file, ome2ass_stats
        )
        if not ome2alia:
            ome2alia = null
...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Summarize CLOCI output for taxonomic rank"
    )
    ...
    parser.add_argument(
        "-a", "--alien", help="Directory of HG alignments to quantify Alien Index"
    )
    parser.add_argument(
        "-g", "--gcf", action="store_true", help="Only run calculations for GCFs"
    )
    ...
    main(
        out_dir,
        ome_dir,
        mtdb(format_path(args.mtdb)),
        rank,
        format_path(args.mtdb),
        alien=format_path(args.alien),
        gcf_only=args.gcf,
        cpus=args.cpus,
    )
