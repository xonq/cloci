...
def cli():
    ...
    parser = argparse.ArgumentParser(
        description="Create table of HLG " + "presence-absence"
    )
    parser.add_argument("-c", "--cloci", help="CLOCI output dir", required=True)
    parser.add_argument(
        "-g", "--gcf", help="Output GCFs instead of HLGs", action="store_true"
    )
    parser.add_argument(
        "-o", "--omes", help="MTDB or list of omes to " + "extract in order"
    )
    args = parser.parse_args()

    if args.omes:
        with open(format_path(args.omes), "r") as raw:
            omes = [x.split()[0] for x in raw if x.rstrip() and not x.startswith("#")]
    else:
        omes = []
