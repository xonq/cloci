...
from mycotools.lib.kontools import collect_files
...
def hash_4_nulls_bysize(gff_path, gene2hg, size, plusminus, ome):
    ...
    for scaf, seqs in cds_dict.items():
        for i, seq in enumerate(seqs):
            ...
            loc_og = set(
                [gene2hg[x] for x in locus if x in gene2hg]
            )  # translate to OGs
            nulls.extend(([tuple(sorted(x)) for x in combinations(loc_og, size)]))

    return ome, set(nulls)


def hash_4_nulls(gff_path, gene2hg, max_size, plusminus, ome=None):
    ...
    for scaf, seqs in cds_dict.items():
        ...
        for size, size_list in lrg_sizes.items():
            size_list.extend(
                [
                    tuple(sorted(seqs[x : x + size]))
                    for x in range(len(scaf))
...
def gen_null_dict_omes(db, i2ome, combo_dict, sample=10000, omes=[]):
    ...
    cooccur_dict = {k: tuple(v) for k, v in cooccur_dict.items() if len(v) > 1}
    # {(OG0,OGn): [omei0...]}
    reps = {k: v for k, v in reps.items() if k in cooccur_dict}
...
def gen_null_dict(combo_dict, sample=10000):
    ...
    try:  # sample
        ...
        logger.warning("\t\t\t\trequested null sample greater than HGx size")
...
def load_partitions(partition_file, ome2i):
    ...
    with open(partition_file, "r") as raw:
        partition_omes = [
            [ome2i[y] for y in x.rstrip().split() if y in omes]
            for x in raw
...
def gen_nulls(
    ...
    if not uniq_sp:
        results = treecalcs.calc_dists(phylo, null_dict, cpus=cpus, func=dist_func)
    else:
        results = treecalcs.calc_dists(
            phylo,
            null_dict,
            cpus=cpus,
            i2ome=i2ome,
...
def gen_pair_nulls(
    ...
    if partition_file or partition_rank:
        ...
        logger.info("\tPartitioning null distributions")
    ...
    if not os.path.isfile(f"{wrk_dir}hgx2loc.pickle"):
        ...
        if not os.path.isfile(f"{nul_dir}2.null.{final_partition}.txt"):
            logger.info("\tGenerating null distributions")
        ...
        for i, pair_null in enumerate(pair_nulls):
            scores_i = round(seed_perc * len(pair_null) + 0.5)
            min_pair_scores.append(pair_null[scores_i])
    else:
        min_pair_scores = None

    return partition_omes, ome2partition, omes2dist, min_pair_scores
    ...
    for size in range(3, max_clus_size + 1):
        ...
        if os.path.isfile(nul_f):
            with open(nul_f, "r") as raw:
                nullSizes = [float(x.rstrip()) for x in raw]
            nullSizes.sort()
            hgxBordPercs[size] = nullSizes[round(hgx_perc * len(nullSizes) + 0.5)]
            hgxClusPercs[size] = nullSizes[round(clus_perc * len(nullSizes) + 0.5)]
...
def parse_skip_files(skip_files):
    ...
    for skip_f in skip_files:
        s, i = [
            int(x) for x in skip_f.replace("skip.", "").replace(".txt", "").split("-")
        ]
        if s > skip_i[i]:
            skip_i[i] = s
...
def gen_hgx_nulls(
    ...
    logger.info("\tCalculating random sample TMD")
    ...
    for size in range(3, max_clus_size + 1):  # for all observed sizes
        if all(i in skip_i for i, v in enumerate(partition_omes)):
            if all(skip_i[i] <= size for i, v in enumerate(partition_omes)):
                ...
                continue
        logger.info(f"\t\tHGx size {size}")
        if not all(
            os.path.isfile(f"{nul_dir}{size}.null.{i}.txt")
        ...
        for i0, omes in enumerate(partition_omes):
            ...
            if os.path.isfile(f"{nul_dir}{size}.null.{i0}.txt"):
                ...
                null_dict, reps = gen_null_dict_omes(
                    db, i2ome, size_dict, samples, omes
                )
                ...
                shutil.move(
                    f"{nul_dir}{size}.null.{i0}.txt.tmp",
                ...
                nullSizes.sort()
            hgxBordPercs[i0][size] = nullSizes[round(hgx_perc * len(nullSizes) + 0.5)]
            hgxClusPercs[i0][size] = nullSizes[round(clus_perc * len(nullSizes) + 0.5)]
            if not hgxBordPercs[i0][size]:
                skip_i[i0] = size
                for s in range(size, max_clus_size + 1):
                    with open(f"{nul_dir}skip.{s}-{i0}.txt", "w") as out:
                        pass
                logger.warning(f"\t\t\tlineage {i0}: all HGx >= {size} HGs pass")
...
def partitions2hgx_nulls(
    ...
    final_partition = len(partition_omes) - 1
    logger.info("\tPreparing HGx nulls")
    ...
    if done_file:
        ...
        for i, omes in enumerate(partition_omes):
            bordScores, clusScores = load_hgx_nulls(
                max_hgx_size, nul_dir, hgx_perc, clus_perc, i
            )
        ...
        bordScores_list, clusScores_list = gen_hgx_nulls(
            db,
            i2ome,
            {db[k]["gff3"]: ome2i[k] for k in list(db.keys()) if k in ome2i},
            gene2hg,
            max_hgx_size,
            plusminus,
            phylo,
