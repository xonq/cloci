...
def merge_protos(protohgx_res):
    ...
    for ome_protohgx in protohgx_res:  # for each protohgx from the mp
        ...
        for hgp, loc_d in ome_protohgx.items():
            ...
            # {(hg0, hg1)}: {hg: i}}
    hgp2hgs = {
        hgp: {v: i for i, v in enumerate(sorted(set(hgs)))}
...
def load_prehgx(wrk_dir, ome2i):
    ...
    preh_fs = [x for x in pre_files if x.startswith("prehgx2data")]
    ...
    h2o = {
        k: tuple(sorted(set(ome2i[y[: y.find("_")]] for y in v)))
...
def formulate_hgx_dicts(hgp, loci, hgs2i, Q):
    ...
    clan_arr = np.zeros([len(loci), len(hgs2i)], dtype=np.int8)
...
def id_hgx(db, hgp_dict, gene2hg, ome2i, wrk_dir, cpus, clusplusminus=10):
    ...
    logger.info("\t\tForming proto-HGx hashes")
    for gff in gffs:  # prepare for protohgx hashing by organism
        hash_protohgx_cmds.append(
            (
                gff,
                hgp_dict,
                gene2hg,
    ...
    if protohgx2omes:
        ...
        if cpus == 1:
            logger.error("need more than one CPU")
        if cpus == 2:
            c_cpus = 1
            w_cpus = 1
        elif cpus < 3:
    ...
    #   logger.info('\t\tRemoving subset HGxs')
...
def hgp2hgx(db, wrk_dir, top_hgs, gene2hg, ome2i, phylo, plusminus=3, cpus=1):
    if not os.path.isfile(wrk_dir + "hgx2loc.pickle"):
        hgp_dict = form_hgpDict(top_hgs)
        logger.info("\tForming HGxs")
        form_clus_start = datetime.now()
        hgx2omes, hgx2loc = id_hgx(
            db, hgp_dict, gene2hg, ome2i, wrk_dir, cpus, clusplusminus=plusminus
        )
