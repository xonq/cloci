...
try:
    ...
    logger.error("graph-tool is not installed. Please install via conda")
    sys.exit(147)
...
def hash_hgx(gff_path, ome, hgx_genes, gene2hg, clusplusminus):
    ...
    cds_dict = input_parsing.compileCDS(
        ...
        for i0, seq0 in enumerate(cds_dict[scaf]):
            if seq0 in hgx_genes:  # if the og is part of a significant seed
                ...
                for hgx in hgx_genes[seq0]:
                    ...
                    try:
                        hgx_dict[hgx][og0].append(
                            seq0
                        )  # add the sequence to the hgx_dict
                    except KeyError:
                        pass
                    start, end = None, None
    ...
    gene_tup = tuple(
        [tuple([gene, tuple(sorted(vals))]) for gene, vals in gene_dict.items()]
    )
    # ((gene, {gene: [HGx...]}))
        ...
        [
            tuple(
                [
                    ...
                    tuple(
                        [
                            tuple([og, tuple(sorted(set(seqs)))])
...
def MCL(adj_path, clus_file, inflation=1.5, threads=1, rows_file=None):
    ...
    subprocess.call(
        ["mcxdump", "-icl", clus_file + ".tmp", "-o", clus_file, "--dump-pairs"],
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    )
...
def calc_loc2loc_sim_mat(
    i0, i1, index, loc0, loc1, hgL0, hgL1, coef, blast_ids, min_hlg_id=0.15
):
    ...
    hg_dict = {k: v for k, v in hg_dict.items() if v[0] and v[1]}
    ...
    for hg in inter_list:
        ...
        for gene0 in hg_dict[hg][0]:
            for gene1 in hg_dict[hg][1]:
                ...
                #                       logger.info('mycste1_96440 recovered 4, no error',
                #                            gene0, gene1)
                # if the genes are missing its a 0
                # the alignment algorithm MUST output enough alignments
                # for this is to reliably work on widely dispersed clusters
                    ...
                    scores[-1].append(0)
    #                  if any(x == 'mycste1_96440' for x in [gene0, gene1]):
    #                     logger.info('mycste1_96440 recovered 5, error',
    #                          gene0, gene1)
...
#       logger.info(f'\t\t\t\t\tAttempting to BLAST {hg}')
    ...
    with open(algn_base, "r") as raw:
        for line in raw:
            ...
            try:
                ...
                logger.error("malformatted alignment: {os.path.basename(algn_base)}")
                return False
...
def calc_sim_by_row(
    adj_mat, hgLoci, loci, blast_ids, index, Q, max_complete, min_hlg_id, simfun
):
...
def matrix_sim_mngr(
    s_mat, out, loci, hgLoci, max_complete, blast_ids, index, min_hlg_id
):
...
def rnd2_loc2loc_mngr(
    hlg_dir, loci, hg_loci, hgx_dir, groups, minid, min_hlg_id, simfun, cpus=1
):
    ...
    for g, v in enumerate(groups):
        ...
        for i2 in range(i0, i1):
            ...
            #            for x in locus:
            #               if "mycste1_96440" == x:
            #                  logger.info('mycste1_96440 recovered 3')
            #                 error_group = g
    ...
    logger.info("\t\t\t\tLoading alignment results")
    if not os.path.isdir(f"{hlg_dir}algn/"):
        os.mkdir(f"{hlg_dir}algn/")
    jsons = collect_files(f"{hlg_dir}algn/", "json")
    finished_hgs = set(int(os.path.basename(x).replace(".json", "")) for x in jsons)
...
def rnd1_loc2loc_mngr(
    ...
    if cpus < 2:
        for hg, genes in blast_hash.items():
            ...
            if gene_len > 1:
                ...
                algn_base = f"{hgx_dir}{hg}.out"
                try:
                    blast_result = parse_blast(algn_base, gene_len, gene_set, minid)
    ...
    if blast_ids:
        if cpus > 1:
            logger.info("\t\t\t\t\tCalculating locus similarity")
            Q.put(None)
        loc2loc_sim_mngr(
            clan_arr,
            finished_file,
...
def merge_clan_loci(
    ...
    clan_loci = {
        clan: list(chain(*list(scaf_dict.values())))
...
def grab_contiguous_singletons(
    singletons, final_loci, loc2hgx, gene2i, hgx2omes, hgx2dist
):
...
def dereplicate_loci(
    ...
    for clan, scaf_dict in preclan_loci.items():
        for scaf, loci in scaf_dict.items():
            ...
            add_loci, new_dp = grab_contiguous_singletons(
                singletons_prep, final_loci, loc2hgx, gene2i, hgx2omes, hgx2dist
            )
            ...
            singletons_prep = [x for x in singletons_prep if x[0] not in genes_set]
            # sort the singletons by TMD, highest to lowest to take the largest
            # branch length representative of the singleton
            sorted_s_prep = sorted(singletons_prep, key=lambda y: y[-2], reverse=True)
            ...
            for i0, locd0 in enumerate(merges):
                ...
                locIntersection = loc0.intersection(loc1)
                if locIntersection:
                    merges[i0 + 1] = [loc0.union(loc1), omes0.union(omes1)]
                merges[i0] = None
            ...
            for locd in final_loci:
                loc, omes = locd[0], locd[-1]
                out_clan_loci[clan][scaf].append(
                    [sorted(loc, key=lambda x: gene2i[x]), omes]
                )
    ...
    for clan, scaf_dict in out_clan_loci.items():
        ...
        for scaf, loci in scaf_dict.items():
            ...
            clan_loci[clan][scaf] = sorted(
                clan_loci[clan][scaf], key=lambda x: scaf2gene2i[scaf][x[0][0]]
            )
    ...
    #   elogger.info('\t\t\tERROR: failed to dereplicate')
            ...
            if seq0 in ome_sig_clus:  # if the og is part of a significant seed
                ...
                for sig_clus in ome_sig_clus[seq0]:
                    ...
                    if i0 < clusplusminus:  # is i0 - clusplusminus < 0 ?
                        ...
                        locus = cds_dict[scaf][
                            i0 - clusplusminus : i0 + clusplusminus + 1
                        ]
                        # instead get the +/- and adjust for python
                    for i1, seq1 in enumerate(locus):  # for each index and sequence
                        # in the locus
                    ...
                    preclan_loci[clan][scaf].append([locus[start:end], hgx])

    out_clan_loci, out_hg_loci = merge_clan_loci(
        preclan_loci,
        gene2hg,
        hgx2omes_path,
        hgx2dist_path,
...
def check_tune(tune, hlg_hgxs, hlg_omes):
    ...
    if failed:
        for name in failed:
            logger.info(f"\t\t\t{name} was not recovered")
    else:
        logger.info(f"\t\t\tTuned successfully")
        satisfied = True
    return satisfied
...
def read_clus(clus_f, mci2pre):
    ...
    return [v for k, v in sorted(t_hlgs.items(), key=lambda x: x[0]) if len(v) > 1]
...
def mcl2hlgs(
    hlg_dir, loci, hgxXloci, ome2i, inflation, loc2clan, tune=False, min_omes=1, cpus=1
):
    satisfied = False
    if not os.path.isfile(hlg_dir + "loci.clus") or tune:
        logger.info(f"\t\t\tRunning MCL - inflation: {inflation}")
        if cpus > 5:
            ...
            if len(set(hlgOme_list)) > min_omes:  # need more than 1
                ...
                if hlgs[-1]:
                    ...
                    for i, omeI in enumerate(hlgOme_list):
                        locus, locI = locs[i]
                        out.write(
                            f'{omeI} {locI} {loc2clan[locI]} {prehlg} {",".join(locus)}\n'
                        )
                else:
                    del hlgs[-1]
                    del hlg_hgxs[-1]
...
def parse_and_sort_gff(gff_path):
    ...
    cds_dict = input_parsing.compileCDS(
        gff2list(gff_path), os.path.basename(gff_path).replace(".gff3", "")
    )
    scaf2gene2i = {}
    for scaf, prots in cds_dict.items():
        scaf2gene2i[scaf] = {}
        ...
        for scaf, locs in scaf_dict.items():
            while len(locs) > 1:
                ...
                # award multiple intervening genes if they meet the max_between criterium
                    ...
                    locs[0].extend(
                        [
                            scaf2i2gene[scaf][top_i + bw_i]
                            for bw_i in range(1, max_between + 1)
                        ]
                        + locs[1]
                    )
                    locs.pop(1)
...
def lg_loc_mngr(db, lgs, gene2hg, max_between=2, cpus=1):
    ...
    with mp.Pool(processes=cpus) as pool:
        lg_loci = pool.starmap(
            hash_ome_lg_loc,
            tqdm(
                ((db[ome]["gff3"], omes2loci[ome], max_between) for ome in db.keys()),
                total=len(db),
            ),
        )
...
def refine_group(
    ...
    protog2h = {
        gI: Counter(chain(*hg_locs)) for gI, hg_locs in enumerate(group_hg_loci)
    }
    group2hgx = {
        gI: set([k for k, v in h.items() if v > 1]) for gI, h in protog2h.items()
    }
    ...
    if rnd != 2:
        ...
        for gI, locs in enumerate(group_loci):
            ...
            for i, locus in enumerate(locs):
                ...
                hgxXloci[index] = tuple(
                    sorted(
                        [
                            x
                            for x in list(hg_loc.intersection(group2hgx[gI]))
                            if x is not None
                        ]
                    )
                )
        ...
        if not os.path.isfile(adj_mtr):
            W = mp.Process(target=write_adj_matrix, args=(adj_mtr, grp_dir, min_loc_id))
            W.start()
            #        logger.info(f'\t\t\t{max([len(x) for x in )} loci in clan 0')
            procs = []

            if cmds:
                if len(cmds[0][2]) > 30000 and cmds[0][1] == 0:
                    logger.info(f"\t\t\t\tRunning group 0")
                    Q = mp.Manager().Queue()
                    procs = [
                        mp.Process(
                            target=rnd1_loc2loc_mngr, args=(*cmds[0], cpus - 1, Q)
                        )
                    ]
                    procs[0].start()
                    Q.get()
        ...
        for gI, locs in enumerate(group_loci):
            ...
            for i, locus in enumerate(locs):
                ...
                #              for x in locus:
                #                  if "mycste1_96440" == x:
                #                       logger.info('mycste1_96440 recovered 2')
                ...
                index += 1
            groups[-1][1] = index
        if not os.path.isfile(f"{grp_dir}loci.adj.tmp") and not os.path.isfile(
            f"{grp_dir}loci.adj"
        ):
            rnd2_loc2loc_mngr(
                grp_dir, loci, hg_loci, hgx_dir, groups, minid, min_loc_id, simfun, cpus
            )
    ...
    while not satisfied:
        if tune:
            ...
            if inflation > 3:
                break
        satisfied, grps, grp_hgxs, grp_omes, lilg2bigg = mcl2hlgs(
            grp_dir, loci, hgxXloci, ome2i, inflation, loc2group, tune, min_omes, cpus
        )
    if tune:
        if not satisfied:
            logger.error("tuning failed to recover clusters")
            sys.exit(135)
        inf_strp = str(inflation * 10)[:2]
        inf_str = inf_strp[0] + "." + inf_strp[1]
        with open(f"{wrk_dir}../log.txt", "r") as raw:
            d = raw.read().replace(
                f"{prefix}_inflation\tNone", f"{prefix}_inflation\t" + inf_str
            )
...
def extend_loc_mngr(
    ...
    return (
        {hlg: tuple([loc[0] for loc in locs]) for hlg, locs in hlgs.items()},
        mod_count,
        cont_count,
    )
...
def classify_hlgs(
    ...
    if not os.path.isfile(groupI):
        logger.info("\tAggregating HGxs with overlapping loci")
        hgx_genes = {}
        ...
        for ome in hgx_genes:
            hashOgx_cmds.append(
                [db[ome]["gff3"], ome, hgx_genes[ome], ome2gene2hg[ome], clusplusminus]
            )
        ...
        logger.info("\t\t" + str(datetime.now() - start))
    else:
        logger.info("\tLoading aggregated HGxs")
    ...
    if not os.path.isfile(groupII):
        ...
        for ome, gene2hgx_ome in tqdm(gene2hgx.items(), total=len(gene2hgx)):
            tpairDict = find_hgx_pairs(
                gene2hgx_ome, ome, hgx2i, tpairDict, min_hgx_overlap=min_hgx_overlap
            )
        ...
        logger.info("\tAggregating HGxs")
        # populate a lil_matrix here, then use that network to identify modules
        logger.info("\t\tBuilding binary HGx-HGx network")
        ...
        for idPair, omes in tqdm(pairDict.items(), total=len(pairDict)):
            ...
            matrix[i0, i1] = True
            matrix[i1, i0] = True
        logger.info("\t\tIsolating subgraphs (HGx clans)")
        ...
        omes2dist = treecalcs.update_dists(
            phylo,
            {clanHGxs[i]: omes for i, omes in enumerate(clanOmes)},
            cpus=cpus,
            omes2dist=omes2dist,
            func=dist_func,
            uniq_sp=uniq_sp,
        ...
        logger.info("\tLoading HGx clans")
        with open(groupII, "rb") as in_:
            clans, clanOmes, clanHGxs = pickle.load(in_)

    logger.info("\t\t" + str(len(clans)) + " clans")
    logger.info("\tClassifying HGxs into locus domains")
    clanFile = wrk_dir + "clan2loci."
    if not os.path.isfile(clanFile + "hg.json.gz"):
        logger.info("\t\tPreparing locus extraction")
        ...
        clan_hg_loci = {
            int(k): tuple(v) for k, v in read_json(clanFile + "hg.json.gz").items()
        }
    ...
    clan_loci = {
        k: v
        for k, v in sorted(clan_loci.items(), key=lambda x: len(x[1]), reverse=True)
    }
    ...
    logger.info("\t\tOutputting HG fastas")
    ...
    if None in hg_count:
        del hg_count[None]
    hgs = sorted([x for x, v in hg_count.items() if v > 1])
    hg_dir = input_parsing.hg_fa_mngr(wrk_dir, None, hgs, db, hg2gene, cpus=cpus)
    evo_conco.run_blast(
        hgs,
        db,
        hg_dir,
    ...
    logger.info("\t\tCalling locus domains (sub-cluster domains)")
    clan_lens = [len(v) for v in clan_loci]
    #    logger.info('\t\t\t' + str(sum([len(v) for v in list(clan_loci.values())])) \
    logger.info(f"\t\t\t{sum(clan_lens)}" + " loci")
    logger.info(f"\t\t\t{max(clan_lens)}" + " loci in clan 0")
    ...
    logger.info("\t\t\t" + str(len(lgs)) + " domains")

    logger.info("\t\tMerging loci")
    # error check
    # for k, v in lgs.items():
    #     for x in v:
    #          if not isinstance(x, bool):
    #               if 'mycste1_96440' in set(x[0]):
    #                    logger.info('mycste1_96440 recovered 0')
    ...
    #                    logger.info('mycste1_96440 recovered 1')

    lg_locs = {
        k: v for k, v in sorted(lg2locs.items(), key=lambda x: len(x[1]), reverse=True)
    }
    lg_hg_loci = [lg2hg_locs[k] for k in lg_locs]
    lg_locs = [v for v in lg_locs.values()]

    logger.info("\t\tCalling HLGs (unfiltered-GCFs)")
    lg_lens = [len(v) for v in lg_locs]
    #    logger.info('\t\t\t' + str(sum([len(v) for v in list(clan_loci.values())])) \
    logger.info(f"\t\t\t{sum(lg_lens)}" + " loci")
    logger.info(f"\t\t\t{max(lg_lens)}" + " loci in LG 0")
    ...
    logger.info("\t\t\t" + str(len(hlgs)) + " HLGs")

    logger.info("\tExtending incomplete loci and contig edge loci")
    hlgs, mod_count, cont_count = extend_loc_mngr(
        db,
        hlgs,
        hlg_hgxs,
        ome2gene2hg,
    ...
    logger.info(f"\t\t{mod_count} loci extended")
    logger.info(f"\t\t{cont_count} loci merged from contig edges")

    omes2dist = treecalcs.update_dists(
        phylo,
        {hlg_hgxs[i]: omes for i, omes in hlg_omes.items()},
        cpus=cpus,
        omes2dist=omes2dist,
        func=dist_func,
        uniq_sp=uniq_sp,
